...
let getCouponList = async () => {
  ...
  for (owner of owners) {
    ...
    let notUsedCoupons = await couponRef
      .doc(owner.id)
      .collection("coupons")
      .where("use", "==", "N")
      .get();
    ...
    for (const notUsedCoupon of notUsedCoupons.docs) {
      ...
      coupons.push({
        uid: owner.id,
        trid: notUsedCoupon.id,
        use: notUsedCoupon.data()["use"],
      });
    }
  }
...
let updateLeaderBoard = async (data) => {
  ...
  let userQuery = db
    .collection("users")
    .where("earned_honey", ">", 0)
    .orderBy("earned_honey", "desc")
    .limit(100);
  let rank = 0;

  try {
    let ranks = await rankCollection.get();
    let rankUsers = await userQuery.get();

    await db.runTransaction(async (transaction) => {
      // 저장되어있는 ranks document 삭제
      for (const rank of ranks.docs) {
        await transaction.delete(rank.ref);
      }
    ...
    console.log("success updateLeaderBoard");
  } catch (error) {
    console.error(error);
  }
};
